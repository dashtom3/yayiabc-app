...
<div class="header">
      ...
      <div class="headerTitle">余额提现</div>
    </div>
  <div class="saleWithdraw-container">
    <div class="withdraw-pending" v-if="!withdrawSuccess">
      ...
      <div class="withdraw-btn-container">
        <mu-raised-button :label="withdrawOperation?'申请提现':'请等待上次提现审核通过'" fullWidth
                          :color="withdrawOperation?'#fff':'#333'"
                          :backgroundColor="withdrawOperation?'rgb(54, 118, 182)':'#e6e6e6'"
                          :class="[{withdrawOperation:'login-btn-success'},'logout-btn']"
                          @click="submitWithdraw" :disabled="!withdrawOperation"/>
      </div>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        withdrawOperation: true //是否可以进行提现操作
      }
    },
    components: {
    },
     ...
     goBack(){
      ...
      async submitWithdraw() {
        ...
        if (res.errorCode === 'No_Error') {
          ...
          // let d = new Date(this.saleInfo.created)
          let d = new Date()
      ...
      goPage(name) {
        sessionStorage.setItem("iptInfo", [this.submitWitParams.vCode,this.submitWitParams.moneyNnm]);
    ...
    created() {
      ...
      this.submitWitParams.phone = phone
      var iptInfoArr = sessionStorage.getItem('iptInfo')
      this.submitWitParams.vCode = iptInfoArr.split(',')[0];
      this.submitWitParams.moneyNnm = iptInfoArr.split(',')[1];
      sessionStorage.setItem("iptInfo",'');
      this.$store.dispatch('sale/SALEINFO_QUERY')
      this.withdrawOperation = (this.saleInfo.describey + '').includes('提现申请中') ? false : true
      if (!this.withdrawOperation) {
        this.withdrawSuccess = true
        this.submitWitParams.moneyNnm = this.saleInfo.describey.split(',')[0].substr(6)
        console.log(this.saleInfo)
        let d = new Date(this.saleInfo.created)
        this.withdrawTime = `${d.getFullYear()}年${(d.getMonth() + 1).toString().padStart(2, '0')}月${d.getDate().toString().padStart(2, '0')}日　　${d.getHours().toString().padStart(2, '0')}:${d.getMinutes().toString().padStart(2, '0')}:${d.getSeconds().toString().padStart(2, '0')}`
        // MessageBox.alert(`您上次提现${(this.saleInfo.describey + '').match(/\d+/)[0]}元正在申请中`)
      }
    },
    beforeDestroy() {
      clearInterval(this.timer)
